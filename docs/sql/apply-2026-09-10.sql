...
-- fails quietly if shuffled: 02 creates the exam_definitions rows that 03,
-- 04 and 05 attach their question banks to by slug, and a bank insert that
-- finds no matching exam inserts nothing -- no error, no questions, and an
-- exam that stays switched off for no visible reason.
--
-- 01 and 06 are independent of the rest. 01 adds the two columns that let a
-- processed course become a study deck; 06 adds the column that stores a
-- raw-marks result for JEE and NEET.
--
-- Every statement is idempotent. Running this twice is safe, and re-running
-- it after you have already applied 01 is safe too.
