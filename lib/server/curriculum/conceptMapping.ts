...
export async function extractConceptsFromChapterSummary(args: {
  ...
  const prompt = `
You are building a curriculum knowledge graph for AceDecks from an already-extracted chapter summary. Break this chapter down into its distinct teachable concepts (roughly topic/skill-sized -- not every single fact, and not just one concept for the whole chapter).
