...
export async function summarizeChunksBatch(
  ...
  const prompt = `
You are extracting structured study content from textbook excerpts for AceDecks's curriculum engine.
...
export async function summarizeFromChildren(args: {
  ...
  const prompt = `
You are synthesizing a ${args.levelLabel}-level summary for AceDecks's curriculum engine, from the summaries of its parts (already extracted, don't re-derive from scratch -- merge and deduplicate them).
