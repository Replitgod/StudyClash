...
function profileFactsBlock(profile: MarketingProductProfile): string {
  ...
  push("AceDecks Pro", profile.pro_details);
...
- Never say AceDecks guarantees higher grades or better test scores.
- Never claim "thousands of users" or any user-count claim unless that exact number is in the verified stats above.
- Be honest that this is a real product built by a real person -- when the destination expects a personal voice (Reddit, founder communities, beta communities, LinkedIn), disclose that you (the poster) built AceDecks.
...
- Honestly disclose that you built AceDecks.
...
- A plain-language explanation of what AceDecks is.
...
"title" is the post title. "body" is what AceDecks is, in plain terms, plus the product link placed naturally.
`.trim(),
...
"body" must cover, in plain language: what AceDecks does, why a student might want to use it, how to try it, and what feedback is wanted.
...
export function buildMarketingPrompt(args: {
  ...
  return `
You are drafting marketing/outreach content for AceDecks, written by its founder for their own review before posting anywhere. This is a real product with a real person behind it -- not a spam bot, not a content mill.

=== AceDecks facts (use ONLY these facts, never invent additional claims) ===
