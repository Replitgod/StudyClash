...
export default function DemoBattlePage() {
	...
	const handleCopyDemoResult = async () => {
		const shareMessage = `I scored ${accuracyPercent}% on the AceDecks Algebra Skills Battle demo. Try to beat me: ${window.location.origin}/demo/battle`;
				...
				{phase === "intro" && (
					<section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-sm sm:p-8">
						...
						<p className="mx-auto mt-4 max-w-2xl text-center text-sm leading-relaxed text-white/60 sm:text-base">
							This Algebra Skills Battle shows the full AceDecks loop: answer questions, see your score, review weak topics, and jump straight into topic-specific study links. Inspired by common high-school algebra skills — not an official SAT practice test.
						</p>
						...
						<p className="mx-auto mt-3 max-w-xl rounded-xl border border-indigo-400/20 bg-indigo-500/10 px-4 py-2 text-center text-xs font-semibold text-indigo-100 sm:text-sm">
							No account needed. Tap Start Guided Demo and answer {QUESTIONS_PER_DEMO} questions to see the full loop.
						</p>
				...
				{phase === "quiz" && currentQuestion && (
					<section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-sm sm:p-8">
						...
						<div className="mt-6">
							...
							{selectedChoice && (
								<div role="status" aria-live="polite" className="mt-5 rounded-2xl border border-white/10 bg-black/30 p-4">
				...
				{phase === "results" && (
					<section ref={resultsRef} className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-sm sm:p-8">
						...
						<p className="mx-auto mt-3 max-w-2xl text-center text-sm text-white/55 sm:text-base">
							Here is the same AceDecks feedback loop you get instantly, without making a full deck first.
						</p>
