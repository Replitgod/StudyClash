  ...
  return (
    <Background>
      ...
      {isLoading ? (
        ...
        <>
          <h2 className="mt-10 text-xs font-bold uppercase tracking-[0.25em] text-emerald-300">Available now</h2>
          <div className="mt-3 grid gap-4 md:grid-cols-2">
            ...
            {available.map((exam) => (
              <Link
                ...
                className="rounded-2xl border border-emerald-400/25 bg-emerald-500/[0.06] p-5 backdrop-blur-sm transition-colors hover:border-emerald-300/40"
              >
                <div className="flex items-center justify-between gap-2">
                  <p className="text-lg font-bold text-white">{exam.name}</p>
                  <span className="rounded-full border border-emerald-300/40 bg-emerald-400/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-emerald-200">
                    Available
                  </span>
                </div>
