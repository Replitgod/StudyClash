...
export default function DecksPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#05050a] text-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-5xl flex-col items-center px-6 py-16 sm:py-20">
        {/* Badge */}
        ...
        <Link
          href="/create"
          className="group relative mt-8 inline-flex items-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-fuchsia-500 to-violet-600 px-6 py-3.5 text-sm font-bold text-white shadow-[0_0_40px_-10px_rgba(217,70,239,0.6)] transition-transform duration-200 hover:scale-105 active:scale-95"
        >
        ...
        {/* Error state */}
        {!isLoading && loadError && (
          <div className="mt-10 w-full max-w-md rounded-2xl border border-red-400/30 bg-red-500/10 p-6 text-center">
        ...
        {!isLoading && !loadError && decks.length > 0 && (
          <div className="mt-12 grid w-full grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {decks.map((deck) => (
              <div
                key={deck.id}
                className="group flex flex-col justify-between rounded-2xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-sm transition-colors duration-200 hover:border-fuchsia-400/30 hover:bg-white/[0.06]"
              >
                <div>
                ...
                <Link
                  href={`/battle/${deck.id}`}
                  className="group/btn relative mt-6 flex items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-fuchsia-500 to-violet-600 px-6 py-3 text-sm font-bold text-white shadow-[0_0_30px_-10px_rgba(217,70,239,0.5)] transition-transform duration-200 hover:scale-[1.02] active:scale-95"
                >
                  <span className="relative z-10">Play Battle</span>
                  <svg
                    className="relative z-10 h-4 w-4 transition-transform duration-200 group-hover/btn:translate-x-1"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2.5}
